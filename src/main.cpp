...
GyverNTC ntc(AS_PIN_NTC, AS_NTC_RESISTOR, AS_NTC_BETA_COEF, AS_NTC_BASE_TEMP,
             AS_NTC_BASE_RESISTANCE, AS_NTC_RESOLUTION);
...
// Читаем температуру
void read_temp() { sens_temp = ntc.getTempAverage(); }
...
void change_mode() {
  mode++;
  if (mode > AS_MODE_SWITCH_MAX) mode = AS_MODE_SWITCH_MIN;
...
// Функция обработки программного прерывания для энкодера. Читайте
// https://github.com/GyverLibs/EncButton
void enc_isr() { enc.tickISR(); }
